...
3. 以下の種別ごとの作業を行う。
   ...
   - **同期**: AI Platform の共通ルール・エージェント設定・テンプレートを、対象リポジトリの固有設定を保護して比較し、明示的に同期した範囲だけを更新する。
...
4. `PR作成` が明示された実装、CI 修正、文書化、同期、リリース、依存更新、テスト、またはリファクタだけ、作業ブランチからドラフト PR を作成する。既定ブランチには直接反映しない。
...
- 対応: <実装・CI修正・レビュー・調査・診断・同期・リリース・依存更新・テスト・リファクタ・セキュリティの要約>
